      ...
      poly.addTo(mymap).
        //  bindPopup(`${data[i].properties.name}`);
          ...
          fetch(path).then(function(vastaus) {
           ...
           kunta_id = json.variables[0].valueTexts.indexOf(x);
           kunta_code = json.variables[0].values[kunta_id];
           console.log(kunta_code);
          }).then(fetch(path, asetukset).then(function(vastaus) {
              return vastaus.json();
            }).then(function(json, ) {
             kunta_data = json.data[kunta_id].values[0];
           // let kunta_data2 = json.data[kunta_id].values[1];
             console.log(kunta_data);
           //  console.log(kunta_data2);
            }).catch(function(error) {
              console.log(error);
            })
